const data = `Game 1: 4 red, 5 blue, 4 green; 7 red, 8 blue, 2 green; 9 blue, 6 red; 1 green, 3 red, 7 blue; 3 green, 7 red
...
const dataTest = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
(function formatData(data) {
    ...
    for (let i = 0; i < lines.length; i++) {
        let power = 0;
        const line = lines[i];
        const arrayOfLinesWithGameId = line.split(': ');
        const dataLineWithoutGameId = arrayOfLinesWithGameId[1];
        const draws = dataLineWithoutGameId.split('; ');
        let maxRed = 0;
        let maxGreen = 0;
        let maxBlue = 0;
        for (const draw of draws) {
            ...
            for (const color of colors) {
                const [stringNum, rgb] = color.split(' ');
